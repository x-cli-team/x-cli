...
> ** Parity Gap:** Grok One-Shot has a minimal set of built-in slash commands compared to Claude Code. Custom slash commands (via `.x-cli/commands/`) are not yet supported but are planned for future releases.
